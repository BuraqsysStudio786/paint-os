...
async function main() {
  ...
  for (const [i,name] of roomNames.entries()) {
    ...
    await prisma.visualizerSpace.create({data:{clientId:client.id,name,slug:`${slug(name)}-studio`,roomType:space,space,imageUrl:room.imageUrl,thumbnailUrl:room.imageUrl,maskJson:{imageWidth:1600,imageHeight:1000,masks:visualizerMasks[i]},defaultShadeId:createdShades[i].id,isFeatured:i<4}});
  }
